@extends('layouts.ace',['title'=>'Director | Editar informacion de directores'])
...
$('#menu-persona').addClass('active open');
  ...
  $('#director-edit').addClass('active').removeClass('d-none');
$('#foto').aceFileInput({
    style: 'drop',
            droppable: true,
      ...
      @slot('rules')
       ...
       @slot('submitHandler')
		...
		@component('components.js.ajax')
		    ...
		    @slot('url')
				'{{ route("Director.Director.Update",["alumno"=>"$Persona->id"]) }}'
			@endslot
	        @slot('data')
				formData
			@endslot
